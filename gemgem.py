...
gemgem_path = os.path.join(os.path.dirname(__file__),'images')
def main():
    ...
    for i in range(1, NUMGEMIMAGES+1):
        gemImage = pygame.image.load(os.path.join(gemgem_path,'gem%s.png' % i))
    ...
    # Load the sounds.
    GAMESOUNDS = {}
    GAMESOUNDS['bad swap'] = pygame.mixer.Sound(os.path.join(gemgem_path,'badswap.wav'))
    GAMESOUNDS['match'] = []
    for i in range(NUMMATCHSOUNDS):
        GAMESOUNDS['match'].append(pygame.mixer.Sound(os.path.join(gemgem_path,'match%s.wav' % i)))
